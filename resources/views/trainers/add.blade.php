<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Dodaj trenera') }}</h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg" style="border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('store_trainer') }}">
                        @csrf
                        <div>
                        ...
                        <div class="mt-4">
                            ...
                            <x-jet-input id="level" class="block mt-1 w-full" type="number" name="level" :value="old('level')" />
                        </div>
                        <div class="mt-4">
                            <x-jet-label for="description" value="{{ __('Opis') }}" />
                            <textarea id="description" name="description" rows="2" class="block mt-1 w-full border-gray-300 focus:border-purple-300 focus:ring focus:ring-purple-200 rounded-md shadow-sm">{{ old('description') }}</textarea>
                        </div>
