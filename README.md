...
- **Registracija i prijava** — Registracija putem e-maila i lozinke, verifikacija e-maila (Laravel Jetstream), prijava i odjava
...
- **Upravljanje treningima (workouts)** — Pregled treninga, dodavanje novih i brisanje. Tabela `workouts` vodi evidenciju treninga.
...
- **workouts** — model `Workout`; polja `code`, `date`, `grade`, `description`, `trainer`, `member`.
...
| **members** | Tabela članova (dodatna evidencija): npr. `name`, `year`, `plan`, `height_cm`, `body_fat_percentage`, `photo_path`|
...
**GymApp** — Projekat izrađen u sklopu predmeta Objektno orijentirane baze podataka na Tehničkom fakultetu u Bihaću.
